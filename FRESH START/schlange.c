...
body_t* initsnake(char dificultad,int** otherscore)
{
  ...
  //Limpiamos toda la matriz en caso de que haya basura
      ...
      {
          for(counter2=0;counter2<= MAX_X;++counter2)
          {
      ...
      matrix[(MAX_Y+1)/2][(MAX_X/2)-1].state = ALIVE; //Creamos la cola
      head=&matrix[(MAX_Y+1)/2][MAX_X/2]; //Iniciamos el puntero a la cabeza
      matrix[(MAX_Y+1)/2][MAX_X/2].cola = &matrix[(MAX_Y+1)/2][(MAX_X/2)-1]; //Iniciamos el puntero del cuerpo
      direction=KEY_RIGHT;
...
unsigned int get_position(body_t* slot, char eje)
{
    ...
    switch (eje) {
      case 'X': case 'x'://Nos da la columna de una casilla en una matriz
                          position=((slot-matrix[0])%(MAX_X+1));
                          break;
      case 'Y': case 'y'://Nos da la fila de una casilla en una matriz
                          position=((slot-matrix[0])/(MAX_Y+1));
                          break;
