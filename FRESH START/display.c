...
//  FILE* menu; //Puntero al archivo del menu
  ...
  char direction=KEY_RIGHT; //Varable que indica la direccion de la serpiente
  int i,j;
  body_t** head;  //Puntero a la cabeza de la serpiente
  body_t* snake; //Puntero para imprimir la serpiente
...
/*************** Wait until joystick switch is presed to start ***************/
  ...
  head=initsnake(2); //Creamos la serpiente
...
/******************************************************************************/
  forever
  {
      ...
      if(apple)
      {
          display_write(get_position(*apple,"x"),get_position(*apple,"y"),D_ON);
      }
    ...
    //Debemos verificar que este fuera de la dead y distinto a la direccion actual
      ...
      if(direction)
      {
            ...
            {
              display_write(get_position(snake,"x"),get_position(snake, "y"),D_ON);
            }
            --snake;

            display_update();					// Reflect changes on display matrix
            display_write(get_position(snake,"x"),get_position(snake, "y"),D_OFF);
      }
      else
      {
