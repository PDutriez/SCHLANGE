...
void printfrase(char* frase)
{
    ...
    for(c1=0;frase[c1]!=0;++c1)//Primero cargamos cada letra de la frase
    {
        ...
        {
            collet(MAX_X,4,frase[c1],c2);//Imprimimos la columna de la letra
            matrix2display();
            rotar('L',1);
            usleep(85000);
        }
...
int myindex(char letra)
{
      ...
      {
          index=letra-'0'+IND_NUM;//Indice para los numeros
      }
      else if(letra<='Z' && letra>='A')
      {
          index=letra-'A'+IND_LET;//Indice para las letras
      }
      else if(letra<='z' && letra>='a')
      {
          index=letra-'a'+IND_LET;
      }
...
{
    int c1,c2;
    for(c1=0;c1<=MAX_Y;++c1)
    {
        for(c2=0;c2<=MAX_X;++c2)
        {
